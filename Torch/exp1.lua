...
cmd:option('--channelSize', '{128, 256}', 'Number of output channels for each convolution layer.')
...
cmd:option('--dataset', 'Bowl', 'dataset to use : Mnist | NotMnist | Cifar10 | Cifar100 | Svhn | Bowl')
...
elseif opt.dataset == 'Mnist2' then
   datasource = dp.Mnist2{input_preprocess = input_preprocess}
...
elseif opt.dataset == 'Bowl' then
   datasource = dp.Bowl{input_preprocess = input_preprocess}
else
